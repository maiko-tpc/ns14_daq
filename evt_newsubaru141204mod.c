...
void evt(void){
  ...
  /* MADC parameters */
  int nev_madc=0;    // Number of MADC32 events in the buffer
  short wcnt_madc=0; // Word counts of the buffer
  ...
  int ip_madc=0;  // Read pointer for MADC32 Data


  /* V775 parameters */
  int evcnt_v775=0; // Event counter read from 0x1024 and 0x1026
  short tmpevcnt_v775=0;
  int nev_v775=0; // Number of V775 events in the buffer
  short wcnt_v775=0; // Word counts of the buffer
  ...
  int ip_v775=0;  // Read pointer for V775 Data
  int ievpt_v775=0; // Event pointer for event building
  ...
  int ievpt_tmb2[TMB2_NMEM][TMB2_NCN]; // Event pointer for event building
  size_t ccnt_tmb2=0,wcnt_tmb2=0;
...
////////////////////////////////////
  ...
  for(imem=0;imem<TMB2_NMEM;imem++){
	...
	if(iev>1){
	  int itmpdif;
	  itmpdif=ievt_tmb2[imem][icn][iev]-ievt_tmb2[imem][icn][iev-1];
	  if(itmpdif!=1){
	    printk("TMB2:%d%d Event number skipped (iev:%d, iev-1:%d).\n",
		   imem,icn,ievt_tmb2[imem][icn][iev],ievt_tmb2[imem][icn][iev-1]);
	    printk("    :Event number has been changed.\n");
	    ievt_tmb2[imem][icn][iev]=ievt_tmb2[imem][icn][iev-1]+1;
	  }
	  else{
  ...
  for(iev=0;iev<nev_madc;iev++){
    printk("EVTCNT:%d: MADC32:%d  V775:%d  TMB2 ",
  ...
  for(iev=0; iev<nev_madc; iev++){
   ...
   //madc32_segdata(MADC32ADR);
    ...
    /* V775 ***********************************************************/
    /* Set seg ID of V775 (device=7, focal=19, detector=7, module=21) */
    init_segment(MKSEGID(NSBL01,GACKO,SSDT,V775));
    while(ievpt_v775<nev_v775){
      if(ievt_v775[ievpt_v775]!=ievt_madc[iev]){
	printk("iev:%d:V775 event number (%d) is not same with MADC32 (%d).\n",
	       iev,ievt_v775[ievpt_v775],ievt_madc[iev]);
	if(ievt_v775[ievpt_v775]>ievt_madc[iev]){
	  printk("V775 skipped the event %d. No V775 data was acquired.\n",
		 ievt_madc[iev]);
	  continue;
	}
	if(ievt_v775[ievpt_v775]<ievt_madc[iev]){
	  ip_v775+=nw_v775[ievpt_v775++];
	  printk("MADC32 skipped the event %d. V775 data was aborted.\n",
		 ievt_madc[iev]);
	  if(ievpt_v775==nev_v775){
	    printk("No MADC32 event was acquired. Evcnt_madc (%d) is rescaled "
		   "to evcnt_v775 (%d).\n",evcnt_madc,evcnt_v775);
	    evcnt_madc=evcnt_v775;
	    break;
	  }
	  continue;
	...
	memcpy((unsigned int *)(data+mp),&tmpdata_v775[ip_v775],
	       4*nw_v775[ievpt_v775]);
	np=2*nw_v775[ievpt_v775];
	mp+=np;
	segmentsize+=np;
	ip_v775+=nw_v775[ievpt_v775++];
	break;
    ...
    for(imem=0;imem<TMB2_NMEM;imem++){
      for(icn=0;icn<TMB2_NCN;icn++) {
	/*************************************************/
	/*(device=7, focal=19, detector=44--47, module=50) */
	...
	while(ievpt_tmb2[imem][icn]<nev_tmb2[imem][icn]){
	  int itmpevt;
	  itmpevt=ievt_tmb2[imem][icn][ievpt_tmb2[imem][icn]]-1;
	  if(itmpevt!=ievt_madc[iev]){
	    printk("iev:%d:TMB2 (%d%d) event number (%d) is not same "
		   "with MADC32 (%d).\n",
		   iev,imem,icn,itmpevt,ievt_madc[iev]);
	    if(itmpevt>ievt_madc[iev]){
	      printk("TMB2 (%d%d) skipped the event %d. "
		     "No TMB2 data was acquired.\n",imem,icn,
		     ievt_madc[iev]);
	      continue;
	    }
	    if(itmpevt<ievt_madc[iev]){
	      ievpt_tmb2[imem][icn]++;
	      printk("MADC32 skipped the event %d. TMB2 (%d%d) data was aborted.\n",
		     ievt_madc[iev],imem,icn);
	      if(ievpt_tmb2[imem][icn]==nev_tmb2[imem][icn]){
		printk("Critical erros in the data acquisition. "
		       "No MADC32 and V775 events were acquired.\n");
		break;
	      }
	      continue;
	    }
	    ...
	    int itmpevpt;
	    itmpevpt=ievpt_tmb2[imem][icn];
	    ccnt_tmb2=(ip_tmb2[imem][icn][itmpevpt+1]
		       -ip_tmb2[imem][icn][itmpevpt])*4;
	    wcnt_tmb2=ccnt_tmb2/2;
	    memcpy((char *)(data+mp),
		   (char *)(tmb2data[imem][icn]
			    +ip_tmb2[imem][icn][itmpevpt]),
		   ccnt_tmb2);
	    mp +=wcnt_tmb2;
	    ...
	    printk("%08x  ",*(tmb2data[imem][icn]+ip_tmb2[imem][icn][itmpevpt]));
	    printk("%08x  ",*(tmb2data[imem][icn]+ip_tmb2[imem][icn][itmpevpt]+1));
	    printk("%08x  ",*(tmb2data[imem][icn]+ip_tmb2[imem][icn][itmpevpt]+2));
	    printk("%08x\n",*(tmb2data[imem][icn]+ip_tmb2[imem][icn][itmpevpt]+3));
#endif
	    ievpt_tmb2[imem][icn]++;
	    break;
	  }
